...
const AdminArtikel = () => {
    ...
    const handleDeleteTag = () => {
        deleteArticleTag(selectedTag.id, () => {
            setArticleTags(
                articleTags.filter((tag) => tag.id !== selectedTag.id)
            );
            closeModal();
        });
    };
    ...
    const openModal = (type, tag = null) => {
        ...
        if (type === "update_article" && tag) {
            ...
            setPreviewImage(
                `http://localhost:3000/api/artikel/image/${tag.gambar}`
            );
        }
